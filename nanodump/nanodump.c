...
BOOL append(Pdump_context dc, PVOID data, unsigned size)
{
...
Pmodule_info write_module_list_stream(Pdump_context dc)
{
	...
	Pmodule_info module_list = find_modules(dc->hProcess, important_modules, ARRAY_SIZE(important_modules), TRUE);
	if (!module_list)
	{
...
PMiniDumpMemoryDescriptor64 get_memory_ranges(Pdump_context dc, Pmodule_info module_list)
{
	...
	while (TRUE)
	{
		NTSTATUS status = NtQueryVirtualMemory(dc->hProcess, (PVOID)current_address, mic, &mbi, sizeof(mbi), NULL);
		...
		// next memory range
		current_address = (PCHAR)base_address + region_size;
	...
	DPRINT("Enumearted %ld ranges of memory", number_of_ranges);
	return ranges_list;
}

PMiniDumpMemoryDescriptor64 write_memory64_list_stream(Pdump_context dc, Pmodule_info module_list)
{
	PMiniDumpMemoryDescriptor64 memory_ranges = NULL;
	ULONG32 stream_rva = dc->rva;
	...
	while (curr_range)
	{
		...
		NTSTATUS status = NtReadVirtualMemory(dc->hProcess, (PVOID)(ULONG_PTR)curr_range->StartOfMemoryRange, buffer, curr_range->DataSize, NULL);
